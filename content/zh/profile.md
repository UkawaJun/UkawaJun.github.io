---
name: 刘政远
nameAlt: Liu Zhengyuan
headline: 化学出身的工程师。做 VR、三维重建，也做数据和 LLM 系统。
location: 吉林 · 长春
status: 2025 届应用化学本科 · 备考大学院 · 与东京研发团队持续合作
photo: assets/portrait.jpg
photoAlt: 刘政远肖像
link.0: mail | 邮箱 | mailto:2106188898@qq.com
link.1: github | GitHub | https://github.com/UukawaJun
link.2: bilibili | 项目视频 | https://space.bilibili.com/321615514
---

本科读应用化学，同时把实验室、嵌入式、三维重建和语言模型做进自己的工作里：管过 VR 实验室，带队拿过物联网省一，也在东京团队里做日本大学院资料的自动搜索。毕业论文用网络药理学和分子对接，看八角茴香黄酮对胃癌靶点的协同作用。

---
id: tokyo
kicker: 东京
title: 大学院资料搜索 Agent
---

清洗日本考学数据，用 LLM 把高价值文件识别率从约 20% 做到 80% 以上。

---
id: thesis
kicker: 毕业论文
title: 90.5 分 · 专业第三
---

八角茴香黄酮抗胃癌：网络药理学与分子对接。2025.03—07。

---
id: lab
kicker: 实验室
title: VR 机械师
---

轴系拆装虚拟仿真，2024.06—12，已在校实验室落地。
